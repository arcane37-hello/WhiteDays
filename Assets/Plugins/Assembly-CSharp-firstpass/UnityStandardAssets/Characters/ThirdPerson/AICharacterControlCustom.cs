...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class AICharacterControlCustom : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			float num = Vector3.Distance(base.transform.position, targets[m_CurrentTargetIndex].position);
			Vector3 vector = targets[m_CurrentTargetIndex].position - base.transform.position;
			vector.y = 0f;
			vector.Normalize();
			vector = vector * Time.deltaTime * m_Speed;
			Debug.DrawLine(targets[m_CurrentTargetIndex].position, targets[m_CurrentTargetIndex].position + Vector3.up * 6f, Color.blue);
			Debug.DrawLine(base.transform.position, base.transform.position + agent.desiredVelocity, Color.yellow);
