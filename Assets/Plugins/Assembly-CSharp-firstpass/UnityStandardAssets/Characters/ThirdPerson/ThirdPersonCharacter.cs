...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
		...
		private void ScaleCapsuleForCrouching(bool crouch)
		{
			...
			RaycastHit hitInfo;
			if (Physics.SphereCast(ray, m_Capsule.radius * 0.5f, out hitInfo, maxDistance, colliderLayer.value))
			{
				m_Crouching = true;
				return;
			}
		...
		{
			if (simulateRootMotion && m_IsGrounded && Time.deltaTime > 0f)
			{
		...
		{
			RaycastHit hitInfo;
			if (Physics.Raycast(base.transform.position + Vector3.up * 0.1f, Vector3.down, out hitInfo, m_GroundCheckDistance, colliderLayer.value))
			{
